...
(function () {
    ...
    function getParams() {
        // because of how datasette lite works, the search params are actually in the hash
        // we need to get the hash, and then get everything after the ? in the hash
        // if no ? then return empty params
        var hash = location.hash;
        var hashParts = hash.split("?");
        if (hashParts.length < 2) {
            return new URLSearchParams();
        }
        // then we need to get the params from the hash
    ...
    function paramsToUrl(params) {
        ...
        // similarly here we need to put the params back into the hash
        // so split the current hash on the ? and then replace the part after the ?
        // with the new params
        var hash = location.hash;
        var hashParts = hash.split("?");
        hashParts[1] = s;
        // then join the parts back together and set the hash
        new_hash = hashParts.join("?");
        // construct the full url by taking the current location, knocking off the hash,
        // and then adding the new hash
        new_url = location.href.split("#")[0] + new_hash;
        // return the new URL
        return new_url;
    }
    ...
    document.body.addEventListener("click", (ev) => {
        ...
        while (target && target != menu) {
            ...
            // for datasette lite need to explicitly close the menu here as well
            // because the page is being overridden rather than reloaded.
            closeMenu();
        }
        if (!target) {
            closeMenu();
