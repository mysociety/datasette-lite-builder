...
from datasette_lite_builder.build import build_all, dir_from_theme
import tempfile


@pytest.fixture(scope="module")
def render_folder():
    # create new folder persistant folder in tempdir

    with tempfile.TemporaryDirectory() as tmpdirname:
        theme_dir = dir_from_theme("default")
        build_all(pathlib.Path(tmpdirname), theme_dir)
        yield tmpdirname


@pytest.fixture(scope="module")
def static_server(render_folder: str):
    process = Popen(
        ["python", "-m", "http.server", "8123", "--directory", render_folder],
        stdout=PIPE,
    )
    retries = 5
    while retries > 0:
